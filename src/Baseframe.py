...
def move():
    ...
    vx=input("Enter the x velocity:")
    vy=input("Enter the y velocity:")
    omega=input("Enter the rotational velocity about z:")
    ...
    while not rospy.is_shutdown():
        while(current_distance < float(distance)):
            ...
            current_distance= math.sqrt(float(vx) *float(vx)  + float(vy)*float(vy))*(t1-t0)
        pub_front_right_steering.publish(0)
        pub_front_left_steering.publish(0)
        pub_rear_left_steering.publish(0)
        pub_rear_right_steering.publish(0)
        
        pub_front_right_wheel.publish(0)
        pub_front_left_wheel.publish(0)
        pub_rear_left_wheel.publish(0)
        pub_rear_right_wheel.publish(0)
        rate.sleep()
